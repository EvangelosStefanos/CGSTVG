...
class CGSTVG(nn.Module):
    ...
    def forward(self, videos, texts, targets, iteration_rate=-1):
        ...
        clips = clips.permute(dims=(1, 2, 0, 4, 3, 5,
                                    6))  # [B, CLIPS, VIEWS, FRAMES_PER_CLIP, C, H, W] -> [CLIPS, VIEWS, B, C, FRAMES_PER_CLIP, H, W]
        clip_indices = torch.reshape(frame_ids,
                                     (self.NCLIPS, self.FRAMES_PER_CLIP))  # TODO: add support for batch size > 1
